<?php
/**
 * Generated lead handler.
 * Delivery credentials and recipients are read from server environment variables.
 */
...
if ($name === '' || $phone === '' || $project === '') {
    lead_fail_redirect('Missing required lead fields.');
}
if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    lead_fail_redirect('Invalid lead email.');
}
...
if ($to === '' || !filter_var($to, FILTER_VALIDATE_EMAIL)) {
    lead_fail_redirect('Lead email recipient is not configured.');
}
