 ...
 * Keeps transport/upload limits out of individual theme renderers.
 * ---------------------------------------------------------
 */
    ...
    function validate_generation_request() {
        ...
        foreach ($maxTextLengths as $key => $max) {
            ...
            if (is_array($_POST[$key])) {
                fail("Invalid value for {$key}.", 400);
            }
            if (strlen((string) $_POST[$key]) > $max) {
                fail("The {$key} field is too long (maximum {$max} characters).", 400);
            }
        }
        ...
        foreach ($jsonFields as $key) {
            ...
            if (is_array($_POST[$key]) || strlen((string) $_POST[$key]) > 100000) {
                fail("Invalid {$key} payload.", 400);
            }
            $decoded = json_decode((string) $_POST[$key], true);
            if (json_last_error() !== JSON_ERROR_NONE || !is_array($decoded)) {
                fail("Invalid {$key} JSON.", 400);
            }
            if (count($decoded) > 100) {
                fail("Too many {$key} entries (maximum 100).", 400);
            }
        }
        ...
        foreach ($_FILES as $field => $payload) {
            ...
            foreach ($tmpNames as $i => $tmp) {
                ...
                if ($error !== UPLOAD_ERR_OK) {
                    fail("Upload failed for {$field}.", 400);
                }
                if (!is_string($tmp) || !is_uploaded_file($tmp)) {
                    fail("Invalid upload for {$field}.", 400);
                }
                $size = (int) ($sizes[$i] ?? 0);
                if ($size <= 0 || $size > $maxBytes) {
                    fail("Each image must be between 1 byte and 8 MB.", 400);
                }

                $originalName = (string) ($names[$i] ?? '');
                $ext = strtolower(pathinfo($originalName, PATHINFO_EXTENSION));
                if (!in_array($ext, ['jpg', 'jpeg', 'png', 'webp'], true)) {
                    fail("Unsupported image type for {$field}. Use JPG, PNG, or WebP.", 400);
                }

                $info = @getimagesize($tmp);
                if ($info === false || empty($info['mime']) || !isset($allowed[$info['mime']])) {
                    fail("The uploaded file for {$field} is not a valid supported image.", 400);
                }
                if (!in_array($ext, $allowed[$info['mime']], true)) {
                    fail("The file extension does not match the image type for {$field}.", 400);
                }

                $width = (int) ($info[0] ?? 0);
                $height = (int) ($info[1] ?? 0);
                if ($width < 100 || $height < 100 || $width > 10000 || $height > 10000) {
                    fail("Image dimensions for {$field} must be between 100 and 10,000 pixels.", 400);
                }

                $imageCount++;
                if ($imageCount > $maxImages) {
                    fail('Too many images. Maximum 30 images per generated page.', 400);
                }
            }
        }

        if (empty($_FILES['slider']['tmp_name'])) {
            fail('At least one slider image is required.', 400);
        }
    }
}
